...
/// @{
#ifndef _MC_WAIT_CONDITION_H
#define _MC_WAIT_CONDITION_H
...
class McMutex;
class McWaitConditionPrivate;


/** Allows waiting and waking for conditions between threads. */
class MCLIBTHREADED_API McWaitCondition 
{
  public:
    ...
    /** Release the locked mutex and wait on the thread event object. The
        ...
        or wakeAll(). This function will return TRUE in this case. 
        A particular time given in milliseconds has elapsed.
        If time is ULONG_MAX (the default), then
        ...
        state. */
    bool wait( McMutex * mutex, unsigned long time = ULONG_MAX );
...
private:
    ...
    McWaitConditionPrivate * d;
};


#endif // _MC_WAIT_CONDITION_H
