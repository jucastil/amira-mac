...
typedef void SoWxViewerCameraTypeChangeCB(void* userData, SoCamera* camera, SoWxViewer* viewer);

// Large Model Viewing notification callbacks for
// frames per second and decimation percentage:
typedef void SoWxViewerFPSCB(float fps, void* userData, SoWxViewer* viewer);
typedef void SoWxViewerDecimationPercentageCB(float percentage, void* userData, SoWxViewer* viewer);
 ...
 *   of a camera to the SoWxRenderArea class. Whenever a new scene is specified with
 ...
 *   viewer type is SoWxViewer::BROWSER then the camera is told to view the supplied
 *   scene graph but is not added beneath that scene graph root. If the viewer type
 *   is SoWxViewer::EDITOR then the camera is added beneath the supplied scene graph
 *   root.
 *   
 ...
 *   viewer will invoke this callback for every wx event it receives. However, unlike
...
class INVENTORWX_API SoWxViewer : public SoWxRenderArea, public SoStereoViewer
{
public:
  ...
  /**
   * Queries whether the seek functionality is enabled for
   ...
   */
  void setCameraTypeChangeCallback( SoWxViewerCameraTypeChangeCB* callback, void* userdata );
   ...
   */
  SoWxViewer::DrawStyle getDrawStyle( SoWxViewer::DrawType type );
  
  /**
   * Sets the current buffering type in the main view (default
   * SoWxViewer::BUFFER_DOUBLE).
   */
   ...
   */
  SoWxViewer::BufferType getBufferingType() 
  { return (SoWxViewer::BufferType)m_guiViewer->getBufferingType(); }
   ...
   */
  void addStartCallback( SoWxViewerCB* f, void* userData = NULL )
   ...
   * Note: The viewer "this" pointer is passed as callback data.
   */
  void addFinishCallback( SoWxViewerCB* f, void* userData = NULL )
   ...
   * Note: The viewer "this" pointer is passed as callback data.
   */
  void removeStartCallback( SoWxViewerCB* f, void* userData = NULL )
   ...
   * Note: The viewer "this" pointer is passed as callback data.
   */
  void removeFinishCallback( SoWxViewerCB* f, void* userData = NULL )
   ...
   */
  void setDecimationStrategy( SoWxViewer::DecimationStrategy strategy );

  /**
   * Gets strategy to use to decide on decimation value.
   */
  SoWxViewer::DecimationStrategy getDecimationStrategy() 
  { return (SoWxViewer::DecimationStrategy)m_guiViewer->getDecimationStrategy(); }
   ...
   */
  void setFramesPerSecondCallback( SoWxViewerFPSCB* callback, void* userData = NULL );
   ...
   */
  void setDecimationPercentageCallback( SoWxViewerDecimationPercentageCB* callback, void* userData = NULL );
  ...
  /**
   * Returns always NULL on UNIX systems.
   */
   ...
   */
  virtual void setCursorStyle( SoWxViewer::CursorStyle style );

  /**
   * Returns the cursor style.
   */
  SoWxViewer::CursorStyle getCursorStyle() 
  { return (SoWxViewer::CursorStyle)m_guiViewer->getCursorStyle(); }
 ...
 protected:
  ...
  wxCursor normalCursor, dollyCursor, panCursor, rollCursor, seekCursor, 
  ...
  SbBool processCommonEvents( wxEvent& xe );
  ...
  // Return a pointer to the wxWindow for stereo
 ...
 private:
  ...
  void constructorCommon( SoWxViewer::Type t, SbBool buildNow );
  void constructorCommon2( SoWxViewer::Type t, SbBool buildNow );
   ...
   */
  static SbBool viewingEventCB( SoWxViewer* viewer, wxEvent& anyEvent );
 
  SoNode* m_cameraSceneGraph;
  SbBool m_runOnce;

  // These members are here only for retro compatibility purposes.
