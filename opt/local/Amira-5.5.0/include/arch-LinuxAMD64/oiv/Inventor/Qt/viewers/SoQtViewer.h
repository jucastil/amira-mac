 ...
 *   of a camera to the SoQtRenderArea class. Whenever a new scene is specified with
 ...
 *   viewer type is SoQtViewer::BROWSER then the camera is told to view the supplied
 *   scene graph but is not added beneath that scene graph root. If the viewer type
 *   is SoQtViewer::EDITOR then the camera is added beneath the supplied scene graph
 *   root.
 *
 ...
 *   viewer will invoke this callback for every Q event it receives. However, unlike
...
class INVENTORQT_API SoQtViewer : public SoQtRenderArea, public SoStereoViewer
{
   ...
   */
  void setCameraTypeChangeCallback( SoQtViewerCameraTypeChangeCB* callback,
                                    void* userdata );
  ...
  { return (SoQtViewer::BufferType)m_guiViewer->getBufferingType(); }
  ...
  */
  ...
  { return (SoQtViewer::DecimationStrategy)m_guiViewer->getDecimationStrategy(); }
   ...
   */
  SoDEPRECATED HDC getCurrentDC() {  return NULL; }
  ...
  { return (SoQtViewer::CursorStyle)m_guiViewer->getCursorStyle(); }
 ...
 SoINTERNAL public:
  ...
  // on the viewer.
  virtual void setNormalVisual( QGLFormat* vi );

  SoDEPRECATED virtual void setNormalVisual( SbGlContextHelper::VisualInfo vi );

  SbBool isInteractive() const;

  void saveSceneGraph( const SbString& fileName );
  ...
  /** 
   * Defines the default (arrow/normal) cursor.
   */
  virtual void defineDefaultCursor( const QCursor& cursor );

  /** 
   * Defines the fly cursor.
   */
  virtual void defineFlyCursor( const QCursor& cursor );

  /** 
   * Defines the rotation (roll) cursor.
   */
  virtual void defineRotationCursor( const QCursor& cursor );

  /**
   * Defines the seek cursor.
   */
  virtual void defineSeekCursor( const QCursor& cursor );

  /** 
   * Defines the translation (pan) cursor.
   */
  virtual void defineTranslationCursor( const QCursor& cursor );

  /** 
   * Defines the up cursor.
   */
  virtual void defineUpCursor( const QCursor& cursor );

  /** 
   * Defines the viewing (spin) cursor.
   */
  virtual void defineViewingCursor( const QCursor& cursor );

  /** 
   * Defines the walk cursor.
   */
  virtual void defineWalkCursor( const QCursor& cursor );

  /**
   * Defines the zoom (dolly) cursor.
   */
  virtual void defineZoomCursor( const QCursor& cursor );
 ...
 protected:
  ...
  QCursor normalCursor, dollyCursor, panCursor, rollCursor, seekCursor,
  ...
  SbBool processCommonEvents( QEvent* xe );
  ...
  // Return a pointer to the QtWidget for stereo
 ...
 private:
  ...
  unsigned int m_zoomBoxPixel;
#endif // UNIX

  void constructorCommon( SoQtViewer::Type t, SbBool buildNow );
  void constructorCommon2( SoQtViewer::Type t, SbBool buildNow );

  SoGuiViewer* m_guiViewer; // Implementation class for SoXxViewer

  // current state vars
  SoQtInputFocus* inputFocus;
  ...
  static void alarmCB( void* thisp, SoSensor* ) { ((SoQtViewer*)thisp)->stillSensorCalled(); }
   ...
   */
  static SbBool viewingEventCB( SoQtViewer* viewer, QEvent* anyEvent );

  // Seek animation
  static void seekAnimationSensorCB( void* p, SoSensor* sensor );

  SoNode* m_cameraSceneGraph;  //SUPPR
};

#endif /* _SO_QT_VIEWER_ */
