...
#ifndef  _SO_VRMLTEXTUREBACKGROUND_
#define  _SO_VRMLTEXTUREBACKGROUND_
 ...
 *   
 *   The SoVRMLTextureBackground node is used to specify a color backdrop that simulates
 ...
 *   SoVRMLTextureBackground nodes are specified in the local coordinate system and are
 *   affected by the accumulated rotation of their parents (see below).
 *   
 *   SoVRMLTextureBackground nodes are bindable nodes (see "Concepts - Bindable Children
 *   Nodes"). There exists an SoVRMLTextureBackground stack, in which the top-most
 *   SoVRMLTextureBackground on the stack is the currently active SoVRMLTextureBackground and thus
 *   applied to the view. To move an SoVRMLTextureBackground to the top of the stack, a TRUE
 *   value is sent to the @B set_bind @b eventIn. Once active, the SoVRMLTextureBackground
 *   is then bound to the browsers view. A FALSE value of @B set_bind @b, removes
 *   the SoVRMLTextureBackground from the stack and unbinds it from the browser viewer. See
 ...
 *   SoVRMLTextureBackground node is subject to the accumulated rotations of its parent
 ...
 *   #groundAngle field. If the last #groundAngle is less than pi@I 
 *    @i(it usually is), then the region between the last #groundAngle and the
 ...
 *   #rightTexture, and #topTexture fields specify a set of images that define a
 ...
 *   orientation as if the image was displayed normally in 2D.
 *   
 ...
 *   
 *   SoVRMLTextureBackground is not affected by SoVRMLFog. Therefore, if an SoVRMLTextureBackground
 *   is active (i.e., bound) while an SoVRMLFog is active, then the SoVRMLTextureBackground
 *   will be displayed with no fogging effects. It is the author's responsibility to
 *   set the SoVRMLTextureBackground values to match the SoVRMLFog (e.g., ground colors fade
 *   to fog color with distance and panorama images tinted with fog color).
 *   
 *   The first SoVRMLTextureBackground node found during reading of the world is
 ...
 *       @TR   SoSFNode   @TD   set_backTexture      
 *       @TR   SoSFNode      @TD   set_bottomTexture      
 *       @TR   SoSFNode   @TD   set_frontTexture      
 *       @TR   SoSFNode   @TD   set_leftTexture      
 *       @TR   SoSFNode   @TD   set_rightTexture      
 *       @TR   SoSFNode   @TD   set_topTexture      
 ...
 *       @TR   SoSFNode   @TD   backTexture_changed      
 *       @TR   SoSFNode   @TD   bottomTexture_changed      
 *       @TR   SoSFNode   @TD   frontTexture_changed      
 *       @TR   SoSFNode   @TD   leftTexture_changed      
 *       @TR   SoSFNode   @TD   rightTexture_changed      
 *       @TR   SoSFNode   @TD   topTexture_changed      
...
class INVENTOR_API SoVRMLTextureBackground : public SoVRMLNode {

  SO_NODE_HEADER(SoVRMLTextureBackground);
 ...
 public:
   ...
   * coordinate system.
   */
  SoSFNode          backTexture;
  /**
   * See #backTexture.
   */
  SoSFNode          bottomTexture;
  /**
   * See #backTexture.
   */
  SoSFNode          frontTexture;
  /**
   * See #backTexture.
   */
  SoSFNode          leftTexture;
  /**
   * See #backTexture.
   */
  SoSFNode          rightTexture;
  /**
   * See #backTexture.
   */
  SoSFNode          topTexture;
   /**
   * Transparency value. A non-zero value allows the underlying
   * window of the containing application to show through.
   * 
   */
 ...
 protected:
  ...
  virtual ~SoVRMLTextureBackground();

  // NOTE: This node has not been fully implemented.
};

#endif /* _SO_VRMLTEXTUREBACKGROUND_ */
