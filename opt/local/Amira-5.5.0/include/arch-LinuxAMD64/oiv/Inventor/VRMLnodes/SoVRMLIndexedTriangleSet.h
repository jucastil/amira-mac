...
#ifndef  _SO_VRMLINDEXEDTRIANGLE_SET_
#define  _SO_VRMLINDEXEDTRIANGLE_SET_
...
//
// An SoVRMLIndexedTriangleSet represents a 3D shape composed of 
// a collection of individual triangles.
//
//////////////////////////////////////////////////////////////////////////////
...
/**
 * Indexed triangle set shape node.\ @NO_ACTION
 * 
 * @ingroup X3D
 * 
 * @DESCRIPTION
 ...
 *
 *   The SoVRMLIndexedTriangleSet node represents a 3D shape composed of 
 *   a collection of individual triangles as depicted in the following figure.
 *
 *  @IMAGE TriangleSet.jpg
 *
 *   SoVRMLIndexedTriangleSet uses the indices in its #index field to specify 
 *   the vertices of each triangle from the #coord field. Each triangle is 
 *   formed from a set of three vertices of the SoVRMLCoordinate node 
 *   identified by three consecutive indices from the #index field. If the 
 *   #index field does not contain a multiple of three coordinate values, 
 *   the remaining vertices will be ignored.
 *   
 *   The SoVRMLIndexedTriangleSet node is specified in the local coordinate 
 *   system and is affected by the transformations of its ancestors. 
 *   Descriptions of the #color, #coord, #normal, and #texCoord fields are 
 *   provided in the SoVRMLColor, SoVRMLCoordinate, SoVRMLNormal, and 
 *   SoVRMLTextureCoordinate nodes, respectively. If values are provided 
 *   for the #color, #normal, and #texCoord fields, the values are applied 
 *   in the same manner as the values from the #coord field and there must 
 *   be at least as many values as are present in the #coord field. 
 *   The value of the #colorPerVertex field is ignored and always treated 
 *   as TRUE. If the #normal field is not supplied, normals will be 
 *   generated as follows:
 *   
 *   If #normalPerVertex is TRUE, the normal at each vertex will be the 
 *   average of the normals for all triangles that share that vertex. 
 *   
 *   If #normalPerVertex is FALSE, the normal at each vertex will be 
 *   perpendicular to the face for that triangle.

 *   The #solid field determines whether the cylinder is visible when 
 *   viewed from the inside. 
 * 
 * @FILE_FORMAT_DEFAULT
 *    VRMLIndexedTriangleSet {
 *    @TABLE_FILE_FORMAT
 *       @TR color             @TD NULL
 *       @TR coord             @TD NULL
 *       @TR normal            @TD NULL
...
class INVENTOR_API SoVRMLIndexedTriangleSet : public SoVRMLTriangle {
  
  SO_NODE_HEADER(SoVRMLIndexedTriangleSet);
 ...
 protected:
  ...
  virtual ~SoVRMLIndexedTriangleSet();
...
#endif /* _SO_VRMLINDEXEDTRIANGLE_SET_ */
