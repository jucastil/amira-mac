...
* 
* @DESCRIPTION
*   SoDirectionalLightManip is the base class for all SoDirectionalLight nodes that
*   have a built-in 3D user interface (this is the only such class provided with the
*   Open Inventor toolkit). Since it is derived from SoDirectionalLight, any changes
*   to its fields result in a change of lighting for nodes that follow it in the
*   scene graph. In this case, the interface edits the @B direction @b field. Also,
*   the color of the manipulator's geometry will reflect the color of the light (but
*   you cannot edit the color using this manipulator).
*   
*   Typically, you will want to replace a regular SoDirectionalLight with an
*   SoDirectionalLightManip (as when the user selects a light to be edited), or vice
*   versa (as when the user is done moving the light and the interface should go
*   away). Use the replaceNode() method to insert a manipulator into a scene
*   graph, and the replaceManip() method to remove it when done.
*   
*   The SoDirectionalLightManip utilizes an SoDirectionalLightDragger to provide a 3D
*   interface. However, the manipulator differs from the dragger; it lights other
*   objects in the scene because, as an SoDirectionalLight, it alters the state. The
*   field values and movement of the dragger, on the other hand, affect only the
*   dragger itself. To find out more about how the interface works and what each
*   part will do, see the reference page for SoDirectionalLightDragger. The
*   interfaces of the dragger and the manipulator are identical.
*   
*   The SoDirectionalLightManip utilizes its dragger by adding it as a hidden child.
*   When an action is applied to the manipulator, such as rendering or handling
*   events, the manipulator first traverses the dragger, and then the manipulator
*   adds its lighting parameters to the state. When you click-drag-release over the
...
*   ensure that the @B direction @b field of the @B manipulator @b will change by
*   the same amount, thus changing the lighting of nodes which follow in the scene
*   graph. Similarly, if you set the @B direction @b field of the
...
*        finished. Then the SoDirectionalLightManip adds a directional light to the state
*        just like its base class, SoDirectionalLight.
* 
*    SoSearchAction @BR
*        Searches just like an SoDirectionalLight. Does not search the dragger, which is
*        a hidden child.
* 
*    SoWriteAction @BR
*        Writes out just like an SoDirectionalLight. Does not write the dragger, which
*        is a hidden child. If you really need to write valuable information about the
...
class INVENTOR_API SoDirectionalLightManip : public SoDirectionalLight {
  ...
  * be an SoDirectionalLight node (or subclass thereof). If the path has a nodekit,
  ...
  * 
  * The field values from the directional light node will be copied to this
  ...
  * SoDirectionalLight node. If the path has a nodekit, this will try to use
  * setPart() to insert the new node. Otherwise, the manipulator requires
  * that the next to last node in the path chain be a group.
  * 
  * The field values from the manipulator will be copied to the directional light
