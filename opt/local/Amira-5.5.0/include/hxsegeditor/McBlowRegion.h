...
/// @{
#ifndef MC_BLOW_REGION
#define MC_BLOW_REGION
...
public:
    ...
    /// constructor
    McBlowRegion(int w, int h,unsigned char* input,
                 int shrink, int gaussWidth);
    McBlowRegion() {};
    /// destructor
    ~McBlowRegion();
...
protected:
    ...
    float getSpeed(McVec2f c);

    /// computes the absolute value of the gradient of given input
    void absOfGradient(unsigned char*  input,float* gradient);
    ...
    McDArray <float> speed;
    McDArray <float> gradient;
    McDArray <McVec2f> gradientOfSpeed;
