...
/// @{
#ifndef IM_LANCZOSFILTER_H
#define IM_LANCZOSFILTER_H

#include <hximproc/HxImprocWinDLLApi.h>
#include <hximproc/ImSepConvFilter3D.h>
#include <hxcore/HxPortFloatTextN.h>

/**The Lanzcos filter can be used to sharpen images. It performs a
    convolution with a Lanzcos kernel*/
class HXIMPROC_API ImLanczosFilter3D : public ImSepConvFilter3D {
  MC_HEADER( ImLanczosFilter3D);

public:
  ///Constructor
  ImLanczosFilter3D(HxEditor* editor=0);
  ...
  /**Calls methods calculateKernel() and ImSepConvFilter3D::convolve(
    class McTypedData2D*, class McTypedData2D*, class McProgressInterface*)*/
...
/**Sigma allows to adjust the Lanczos function.
  Note that the kernel isn`t updated unless calculateKernel() is called */  
  ...
  static ImFilter3D* createFilter(HxEditor* e) {
      return new ImLanczosFilter3D(e); }
