...
/// @{
#ifndef HXSIGMOID2D_H
#define HXSIGMOID2D_H
...
class HXIMPROC_API ImSigmoidFilter2D : public ImFilter2D
{
  MC_HEADER( ImSigmoidFilter2D);

public:
  /// Constructor.
  ImSigmoidFilter2D(HxEditor* editor=0);
  ...
  HxPortFloatTextN portAlphaBeta;

  HxPortFloatTextN portMinMax;

  /// apply filter as documented in ImFilter3D
  void apply2D(McTypedData2D* src,
    McTypedData2D* dst=NULL);
  ...
  static ImFilter3D* createFilter(HxEditor* e) {
    return new ImSigmoidFilter2D(e); }
