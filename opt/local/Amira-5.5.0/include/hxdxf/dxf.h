...
#define InvalidData(s) throw ReadErr(s);
...
/* The working collection of points, colors, normals,... */
typedef McHashTable<struct point*,int> POINTHASH;
typedef McHashTable<int,int> COLORHASH; /* color,nr. */
typedef struct _collection {
    int nPnts;
    int nConns;
    POINTHASH* point_hash;

    POINTHASH* normal_hash;
...
typedef void * (*PFVP)(FILE *fpin, char *s, char *value, int *group, int *type);
typedef void (*PFV)(Entity* e,Collection* c);
...
void doAll			(Entity *e, Collection *c);
...
int find_color_index(Entity *e);
RGBColor* get_color_from_index(int index);
int getACI(float r, float b, float g);
