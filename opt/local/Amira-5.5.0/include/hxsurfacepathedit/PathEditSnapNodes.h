...
/// @{
#ifndef PATHEDIT_SNAP_NODES
#define PATHEDIT_SNAP_NODES

#include <hxsurfacepathedit/hxsurfacepatheditAPI.h>
#include <hxsurfacepathedit/SinglePathOperation.h>

/** Converts the nodes according to a specified tolerance. */
class HXSURFACEPATHEDIT_API PathEditSnapNodes : public SinglePathOperation
{
    MC_ABSTRACT_HEADER(PathEditSnapNodes);

public:

    /// Constructor.
    PathEditSnapNodes(SurfacePathSet* ps);

    /// Set tolerance (see SurfaceNodeFactory).
    void setTolerance(int mode, float tol)
    {
        mMode = mode;
        mTol = tol;
    }
