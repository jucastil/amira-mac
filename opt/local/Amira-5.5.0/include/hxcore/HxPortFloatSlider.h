...
/// @{
#ifndef HX_PORT_FLOAT_SLIDER
#define HX_PORT_FLOAT_SLIDER
...
/** Represents a bounded floating point number which can be edited
    ...
    see the  @htmlonly <a href= ../usersguide/HxPortFloatSlider.html> usersguide </A> @endhtmlonly
    for a detailed view of the port's GUI. */
class HXCORE_API HxPortFloatSlider : public HxPort {
    MC_ABSTRACT_HEADER(HxPortFloatSlider);

  public:
    /// Constructor for objects.
    HxPortFloatSlider(HxObject *obj, const char *name);

    /// Constructor for editors.
    HxPortFloatSlider(HxEditor *editor, const char *name);

    /// Destructor.
    virtual ~HxPortFloatSlider();
    ...
    /// Tells if port should track interactive slider interaction
    void setInteractiveTracking(bool interactiveTracking) { interactiveFlagTracking = interactiveTracking; };

    /// Check if slider is interactive
    bool isInteractive() const { return interactiveFlag; }
  ...
  protected:
    QxGuardedPtr<QxFullSlider> slider;
    McString format;
    ...
    unsigned int interactiveFlagTracking:1;
...
#define HxPortAnimatedFloat HxPortFloatSlider
