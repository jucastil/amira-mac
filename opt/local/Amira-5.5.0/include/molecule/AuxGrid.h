...
/// @{
#ifndef MOLECULE_AUXGRID_H
#define MOLECULE_AUXGRID_H
...
template<class Value> class AuxGrid {
private:
    // Bounding box.
    McBox3f bbox;
    // Bounding box.
...
public:
    ...
    AuxGrid(const McBox3f & bb, float cellLength);
    /// Destructor.
    ~AuxGrid();
...
AuxGrid<Value>::AuxGrid(int d[3])
{
    dims[0] = d[0]; dims[1] = d[1]; dims[2] = d[2];

    int size = dims[0]*dims[1]*dims[2];
    if ( size < 0 || size > 10000000 ) 
        throw McException("Wrong AuxGrid dimension: %d",size);
...
template<class Value> 
AuxGrid<Value>::AuxGrid(const McBox3f & bb, float cellLength)
{
    bbox = bb;
    bboxMin = bbox.getMin();
...
template<class Value> 
void AuxGrid<Value>::computeKey(const McVec3f & pos, int key[3])
{
    int i;
    for (i=0; i<3; i++) {
...
template<class Value> 
void AuxGrid<Value>::setDims(int d[3])
{
    grid.clear();
    dims[0] = d[0]; dims[1] = d[1]; dims[2] = d[2];
    grid.resize(dims[0]*dims[1]*dims[2]);
...
template<class Value> 
void AuxGrid<Value>::insert(int idx[3], Value info) 
{
    grid[idx[0]+dims[0]*(idx[1]+idx[2]*dims[1])].append(info);
}

template<class Value> 
void AuxGrid<Value>::insert(const McVec3f & pos, Value info)
{
    int key[3];
    computeKey(pos, key);
    insert(key, info);
...
template<class Value> 
void AuxGrid<Value>::remove(int idx[3], Value info) 
{
...
template<class Value> 
McDArray<Value>* AuxGrid<Value>::search(int idx[3]) 
{
    return &(grid[idx[0]+dims[0]*(idx[1]+idx[2]*dims[1])]);
}

template<class Value> 
McDArray<Value>* AuxGrid<Value>::search(const McVec3f & pos) 
{
    int key[3];
    computeKey(pos, key);
    return search(key);
...
template<class Value> 
void AuxGrid<Value>::search(int idx[3], int plusMinus, 
                            McDArray<Value> &list) 
{
    int i, j, c;
...
template<class Value> 
void AuxGrid<Value>::search(const McVec3f & pos, int plusMinus, 
                            McDArray<Value> &list) 
{
    int key[3];
    computeKey(pos, key);
...
template<class Value> 
int AuxGrid<Value>::search(int idx[3], int plusMinus, 
...
template<class Value> 
int AuxGrid<Value>::search(const McVec3f & pos, int plusMinus, 
                           McDArray<McDArray<Value> *> &list) 
{
    int key[3];
    computeKey(pos, key);
...
template<class Value> 
int AuxGrid<Value>::search(int idx[3], int pm[6], 
                           McDArray<McDArray<Value> *> &list)
{
    // resize list and initialize all entries to zero
...
template<class Value> 
int AuxGrid<Value>::search(const McVec3f & pos, int pm[6], 
                           McDArray<McDArray<Value> *> &list)
{
    int key[3];
    computeKey(pos, key);
