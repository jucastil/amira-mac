...
/// @{
#ifndef SUBSAMPLED_TRAJECTORY_H
#define SUBSAMPLED_TRAJECTORY_H

#include "MoleculeWinDLLApi.h"
#include "molecule/SubTrajectoryBase.h"

class MolTrajectory;

/** Sub trajectory = topology + time steps.
    All the information in a trajectory file. */
class MOLECULE_API SubsampledTrajectory : public SubTrajectoryBase
{
public:
    /// Create from data handle
    SubsampledTrajectory(MolTrajectory * t, int factor, int offset);
    /// Destructor.
...
private:
    /// Factor and offset determining subsampled trajectory.
    int theFactor, theOffset;
    /// Number of time steps computed from theFactor, theOffset and
    /// number of time steps in main trajectory.
    int numTimeSteps;
};
