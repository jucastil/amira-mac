...
/// @{
#ifndef HX_MAG_AND_PHASE
#define HX_MAG_AND_PHASE
...
/** Computes a color field by using a vector fields magnitude as alpha value
    and the phase as color. */

class HXARITH_API HxMagAndPhase : public HxCompModule
{
  HX_HEADER(HxMagAndPhase);
  ...
  public:
    ...
    /// Constructor.
    HxMagAndPhase(const int dims[3], void* data=0);

    /// Provides button to start actual computation.
    HxPortDoIt portDoIt;

    /// the colormap for the phase values
  ...
  protected:
    /// Destructor.
    virtual ~HxMagAndPhase();

    /// the actual computation
    void computation(unsigned char* dst, const mculong* dims);
