...
/** This module provides casting onto the primitive datatypes
    char, unsigned short, signed int, float and double (not on char *). */

class HXARITH_API HxCastField : public HxCompModule {

  HX_HEADER(HxCastField);
  ...
  public:
    ...
    /// Condense labels (relabel).
    HxPortToggleList portVoxelGridOptions;

    /// Optional material colormap for cast to voxelgrids
    HxPortColormap portColormap;

    /// Condense labels (relabel).
    HxPortMultiMenu portColorFieldOptions;
  ...
  protected:
    ...
    void doCastField();
    void castToVoxelGrid(HxRegScalarField3* src, HxUniformLabelField3* dst);
    void cast(HxLattice3* srclattice, HxLattice3* dstlattice);
    void cast(HxRegColorField3* src, HxRegScalarField3* dst);

    McPrimType dst_datatype;
    double scale, offset;
};
