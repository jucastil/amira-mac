...
/// @{
#ifndef __MCLIST2IBACKWARD_H
#define __MCLIST2IBACKWARD_H

#include "McWinDLLApi.h"
#include "McList2.h"


/** @c McList2IBackward implements a backward iterator for McList2. See
    Iterator for a detailed description of the iterator concept.
    @see McList2IForward
*/

template <class T>
class MCLIB_API McList2IBackward {
    McList2<T>* _list;
...
public:
    ...
    McList2IBackward() : _list(NULL), _cursor(NULL) {}

    /// Attach constructor. Create iterator attached to specified list.
    McList2IBackward(McList2<T>& list) {
	_list = &list;
	_cursor = list.getTail();
    }
    //@}
    ...
    /// Rewind iterator to end of list.
    void rewind() { _cursor = _list->getTail(); }
    ...
    void next() { _cursor = _list->getPrev(_cursor); }
    //@}
};
