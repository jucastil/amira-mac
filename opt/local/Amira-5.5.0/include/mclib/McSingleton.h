...
class McSingleton {
  ...
  protected:
    /// Prevent creation of a new singleton instance.
    McSingleton() {}
    /// Prevent destruction via pointer deletion.
    ~McSingleton() {}

  private:
    /// Disable automatic copy constructor generation by the compiler.
    McSingleton(const McSingleton&);
    /// Disable assignments (which always are self-assignments for singletons).
    McSingleton& operator=(const McSingleton&);
};

template<class C>
inline C& McSingleton<C>::the() {
    static C instance;
    return instance;
}

template<class C>
inline C* McSingleton<C>::getInstance() {
    return &the();
}
