...
/// @{
#ifndef MOL_TRAJECTORY_VDE_H
#define MOL_TRAJECTORY_VDE_H
...
class MOLIO_API MolTrajectoryVDE : public MolTrajectory 
{
private:
    ostream * stream;

    ///
    McDArray<McString> names;

    ///
    McHandle<MolTopology> baseTopo;
...
protected:
    ...
    float offspringObservable(int ix, int step) const { return 0; }
        ...
        ///
    istream * openStep(int index);

    ///
    void readBaseTopo(istream & is);
    
    ///
    void readAtomPositions(istream & is, Molecule * M);

    ///
    void readBonds(istream & is, Molecule * M);
    
public:
    /// Create from data handle
    MolTrajectoryVDE(ostream * str, AtomDataBase * atomDataB);
    ...
    ///
    MolTrajectoryVDE *  readVDEMolTrajectory(int n, const char ** filenames);
};
