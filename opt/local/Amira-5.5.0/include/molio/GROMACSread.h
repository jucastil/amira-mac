...
/// @{
#ifndef GROMACSREAD_H
#define GROMACSREAD_H
...
class TrnHeader : public McHandable 
{
public: 
    TrnHeader();
    ...
    double        lambda; /* Current value of lambda                      */
...
class MOLIO_API GROMACSread
{
public:        
    /// main constructor
    GROMACSread(ostream * str = 0, McProgressInterface * wArea = 0);

    // return a READ Trajectory    
    MolTrajectoryGROMACS * readGROMACSMolTrajectory(const char *fileNameTRJ, const char *fileNameGRO, const char *fileNameTOP = 0);

    Molecule * readGROMACSMolecule(const char * fileNameGRO, const char * fileNameTOP = 0);

    /// return name of file format
    const char * getName() const { return "GROMACS Double Precision"; }

    /** reading coordinates of the trajectory at the timestep ix, at
    the streamposition trajsp in the file FileName */
    TrnHeader * readCoordsAt(Molecule* M, long  trajPos, McString FileName);
    ...
    // checks if the file is a single format or a double format
    static int checkFormat(McString FileName);
};
