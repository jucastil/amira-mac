...
/// @{
#ifndef GROMACSWRITE_H
#define GROMACSWRITE_H
...
class MOLIO_API GROMACSwrite
{
    ostream * stream;
    McProgressInterface * workArea;
public:
    // constructur witht the filename 
    GROMACSwrite(ostream * str = 0, McProgressInterface * wArea = 0);

    // saving a MolTrajectory as Gromacs-Trajectory
    int writeGROMACSMolTrajectory(MolTrajectory * traj, const char* fileName);

    // saving a Molecule as Gromacs Molecule (*.gro and *.top file)
    int writeGROMACSMolecule(Molecule * M, const char* fileName);
};
