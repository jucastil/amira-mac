...
/// @{
#ifndef HX_REGULARVECTORFIELD3_H
#define HX_REGULARVECTORFIELD3_H

#include <hxfield/HxLattice3.h>
#include <hxfield/HxVectorField3.h>

/// Represents a vector field based on a regular lattice.

class HXFIELD_API HxRegVectorField3 : public HxVectorField3
{
  HX_DATA_HEADER(HxRegVectorField3);

  public:
    /// Creates a regular vector field.
    static HxRegVectorField3* create(const int dims[3], McPrimType pType,
        HxCoordType cType, void* data = 0);

    /// Constructor.
    HxRegVectorField3(const int dims[3], McPrimType pType,
	HxCoordType cType, void* data=0);

    /// This constructor uses an existing lattice object.
    HxRegVectorField3(HxLattice3* existingLattice);
    ...
    /// Duplicates the current HxRegVectorField3 object
  ...
  protected:
    ...
    void copyData(const HxRegVectorField3 &source);
  
    /// Computes range.
    virtual void computeRange(float& min, float& max, bool discardUndefinedValues=false);

    // utility function to compute independently of the data type.
    template <class T>
    void computeMagnitudeRange(T* val, mculong nValues, int nDataVar, float& min, float& max, bool discardUndefinedValues);
