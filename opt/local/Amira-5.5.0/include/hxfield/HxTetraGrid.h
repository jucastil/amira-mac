...
/// @{
#ifndef HX_TETRA_GRID_H
#define HX_TETRA_GRID_H

#include <hxcore/HxVertexSet.h>
#include <hxfield/TetraGrid.h>

/// This class combines a standalone TetraGrid with Amira's HxVertexSet.

class HXFIELD_API HxTetraGrid : public HxVertexSet, public TetraGrid {

    HX_HEADER(HxTetraGrid);
  ...
  public:
    ...
    // Constructor.
    HxTetraGrid(McVec3f* coords,int npoints,int tetras[][4],int nTetra);

    HxTetraGrid(const McDArray<McVec3f>& coords, const McDArray<int> tets);
    ...
    /// Reads file in Grape ascii format.
    static int readGrape(const char* filename);

    /// Save grid in Grape ascii format.
    int saveGrape(const char* filename);

    /// Creates grid from AmiraMesh struct.
    static int readAmiraMesh(AmiraMesh* m, const char* filename);

    /// Reads file in AmiraMesh format.
    ...
    /// Add another tetrahedral grid to this one.
    void add(const HxTetraGrid& other, int removeDuplicated=1);
    ...
    static HxTetraGrid* findMatchingGrid(AmiraMesh* m, const char* filename,
	int mustHaveEdges=0);
  ...
  protected:
    ...
    void copyData(const HxTetraGrid &source);
};
