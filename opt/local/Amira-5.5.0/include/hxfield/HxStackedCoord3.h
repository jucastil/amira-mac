...
/// @{
#ifndef STACKEDCOORD3
#define STACKEDCOORD3

#include <hxfield/HxCoord3.h>

/** Class representing stacked axis-aligned coordinates.
	These coordinates are organzied in parallel z-aligned slices.
    Within a slice there are uniform xy-coordinates, but each
    slices may have a different z value. 
*/

class HXFIELD_API HxStackedCoord3 : public HxCoord3
{
  public:
    /// Constructor.
    HxStackedCoord3(const int dims[3]);

    /// Resizes coordinate array.
    virtual void resize(const int dims[3]);

    /// Returns XY bounding box information.
    float* boxXY() { return theBoxXY; }

    /// Returns pointer to z-coordinates.
    float* coordZ() { return theCoordZ; }
