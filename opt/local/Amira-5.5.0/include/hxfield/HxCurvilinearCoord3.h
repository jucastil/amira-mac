...
/// @{
#ifndef HX_CURVILINEAR_COORD3_H
#define HX_CURVILINEAR_COORD3_H
...
/// Class representing curvilinear coordinates.

class HXFIELD_API HxCurvilinearCoord3 : public HxCoord3
{
  public:
    /// Constructor.
    HxCurvilinearCoord3(const int dims[3]);

    /// Resizes coordinate array.
    virtual void resize(const int dims[3]);

    /// Returns pointer to coordinates.
    float* coords() const { return theCoords; }
    ...
    McDArray<mculong>& find(float x, float y, float z) const;

  protected:
    float* theCoords;
    McStaticOctree<mculong> theOctree;
    McStaticQuadtree<mculong> theQuadtree;

    float m_cachedBB[6];
    void getCachedBB(float cachedBB[6]) const;
    void setCachedBB(const float cachedBB[6]);
  private:
