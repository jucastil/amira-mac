...
/// @{
#ifndef HX_SUB_TRAJECTORY_H
#define HX_SUB_TRAJECTORY_H
...
/** The class @c HxSubTrajectory contains a subset of time steps of
    some trajectory. All the information is retrieved from this
    trajectory file. */

class HXMOLECULE_API HxSubTrajectory : public HxMolTrajectory
{
    HX_HEADER(HxSubTrajectory);
...
public:
    ...
    HxSubTrajectory(HxMolTrajectory * t, SubTrajectory * st=0);
    /// Destructor.
    ~HxSubTrajectory();
...
protected:
    /// Change byte order.
    static void swapBytes(void *p_,int size,int cnt);
};
