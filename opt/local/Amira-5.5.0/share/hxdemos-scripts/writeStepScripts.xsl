...
# Demo 0 Step <xsl:value-of select="$whichStep"/>
<xsl:text>
</xsl:text>
	
DemoSequence startDemo [lsearch [DemoSequence getVar demoXMLFilenames] {<xsl:value-of select="$demodir"/>description.xml}]<xsl:text> </xsl:text><xsl:value-of select="number($whichStep)"/> 
</xsl:if>
</xsl:template>
