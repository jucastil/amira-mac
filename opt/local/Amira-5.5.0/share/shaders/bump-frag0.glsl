...
vec3 computeNormal()
{
  ...
  float Hg = getData(indexedTex, newTexCoord);
  // height of the texel directly above the given texel:
  float Ha = getData(indexedTex, newTexCoord + vec2(0.0, 1./dimTextureV));
  // height of the texel directly to the right of the given texel:
  float Hr = getData(indexedTex, newTexCoord + vec2(1./dimTextureU, 0.0));	

  vec3 tNormal;
  if( (Hg <= epsilon0 || Ha <= epsilon0 || Hr <= epsilon0 )
      ||  (Hg >= epsilon1 || Ha >= epsilon1 || Hr >= epsilon1) )
    tNormal = vec3(0, 0, 1.0); 
  else
