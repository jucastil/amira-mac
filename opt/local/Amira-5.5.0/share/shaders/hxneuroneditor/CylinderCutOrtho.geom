...
/*
    Proofs if two vectors are nearly the same.
*/
int vIsV(vec3 x, vec3 y, float error)
{
    if (abs(x.x - y.x) <= error &&
        abs(x.y - y.y) <= error &&
        abs(x.z - y.z) <= error)
        return 1;
    else 
...
void main(void)
{
    ...
    vec4  rot    = vec4(normalize(cross(axis, y_axis)), angle(axis, y_axis));
    ...
    if (vIsV(axis, y_axis, 0.001) == 0)
    {
        rn_y  = rotatePoint(rn_y,  rot.xyz, rot.w);
        rn_z  = rotatePoint(rn_z,  rot.xyz, rot.w);
        raxis = rotatePoint(raxis, rot.xyz, rot.w);
        rd    = rotatePoint(rd,    rot.xyz, rot.w);
    }


    // generate quad

    color           = cylinderCol[0].xyz;
    cylinder        = vec4(m, r);
    rayStart        = -raxis + s * rn_z + r * rn_y;
    rayDirection    = vec4(rd, h);
    rotation        = rot;
